...
public class AttVal extends Object implements Cloneable {
	...
	protected Object clone() {
		...
		if (attribute != null) av.attribute = attribute;
		if (value != null) av.value = value;
		av.delim = delim;
		if (asp != null) {
	...
	public boolean isBoolAttribute() {
		...
		if (attribute != null) {
			if (attribute.attrchk == AttrCheckImpl.getCheckBool()) { return true; }
		}
	...
	public Attribute checkAttribute(Lexer lexer, Node node) {
		...
		if (this.asp == null && this.php == null) this.checkUniqueAttribute(lexer, node);
		...
		if (attribute != null) {
			...
			if (attribute.attrchk != null) attribute.attrchk.check(lexer, node, this);
	...
	public void checkUniqueAttribute(Lexer lexer, Node node) {
		...
		for (attr = this.next; attr != null; attr = attr.next) {
			if (this.attribute != null && attr.attribute != null && attr.asp == null && attr.php == null
					&& Lexer.wstrcasecmp(this.attribute, attr.attribute) == 0) ++count;
		}

		if (count > 0) Report.attrError(lexer, node, this.attribute, Report.REPEATED_ATTRIBUTE);
	}
