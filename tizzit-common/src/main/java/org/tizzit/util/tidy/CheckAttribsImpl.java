...
public class CheckAttribsImpl {
	...
	public static class CheckHTML implements CheckAttribs {
		...
		public void check(Lexer lexer, Node node) {
			...
			for (attval = node.attributes; attval != null; attval = attval.next) {
				...
				if (attribute == AttributeTable.attrXmlns) lexer.isvoyager = true;
			}
		}
	...
	public static class CheckTABLE implements CheckAttribs {
		...
		public void check(Lexer lexer, Node node) {
			...
			for (attval = node.attributes; attval != null; attval = attval.next) {
				...
				if (attribute == AttributeTable.attrSummary) hasSummary = true;
			}
			...
			if (lexer.configuration.XmlOut) {
				...
				if (attval != null) {
					if (attval.value == null) attval.value = "1";
				}
			}
		}
	...
	public static class CheckHR implements CheckAttribs {
		...
		public void check(Lexer lexer, Node node) {
			if (node.getAttrByName("src") != null) Report.attrError(lexer, node, "src", Report.PROPRIETARY_ATTR_VALUE);
		}
	};
	...
	public static class CheckIMG implements CheckAttribs {
		...
		public void check(Lexer lexer, Node node) {
			...
			if (!hasAlt) {
				...
				if (lexer.configuration.altText != null) node.addAttribute("alt", lexer.configuration.altText);
			}

			if (!hasSrc && !hasDataFld) Report.attrError(lexer, node, "src", Report.MISSING_ATTRIBUTE);

			if (hasIsMap && !hasUseMap) Report.attrError(lexer, node, "ismap", Report.MISSING_IMAGEMAP);
		}
	...
	public static class CheckAREA implements CheckAttribs {
		...
		public void check(Lexer lexer, Node node) {
			...
			for (attval = node.attributes; attval != null; attval = attval.next) {
				...
				if (attribute == AttributeTable.attrAlt)
					hasAlt = true;
				else if (attribute == AttributeTable.attrHref) hasHref = true;
			}
			...
			if (!hasHref) Report.attrError(lexer, node, "href", Report.MISSING_ATTRIBUTE);
		}
