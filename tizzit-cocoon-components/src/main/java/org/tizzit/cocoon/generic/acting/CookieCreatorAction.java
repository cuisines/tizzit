	...
	public Map act(Redirector redirector, SourceResolver resolver, Map objectModel, String src, Parameters parameters) throws Exception {
		...
		if (getLogger().isDebugEnabled()) {
			if (cookie.getMaxAge() == 0) {
				...
				getLogger().debug("Cookie '" + name + "' created with value '" + value + "' (version " + cookie.getVersion() + "; will be stored for session duration)");
			} else {
				getLogger().debug("Cookie '" + name + "' created with value '" + value + "' (version " + cookie.getVersion() + "; will expire in " + cookie.getMaxAge() + " seconds)");
			}
		}
