...
//		}
	...
	public void paint(Graphics g, JComponent c) {
		paintMenuItem(g, c, checkIcon, arrowIcon,
			selectionBackground, selectionForeground, defaultTextIconGap);
	}
		...
		Icon arrowIcon, int defaultTextIconGap)
	{
		...
		layoutMenuItem(fm, text, fmAccel, acceleratorText, icon, checkIcon, arrowIcon,
			...
			checkIconRect, arrowIconRect, text == null ? 0 : defaultTextIconGap, defaultIconGap);
			...
			!(menuItem instanceof JMenu && ((JMenu)menuItem).isTopLevelMenu()))
		{
			...
			if(r.width < maxTextValue) {
				...
				p.putClientProperty(TinyMenuItemUI.MAX_TEXT_WIDTH, new Integer(r.width));
			}

			//Compare the accelarator widths.
			if(acceleratorRect.width > maxAccValue) {
				maxAccValue = acceleratorRect.width;
				p.putClientProperty(TinyMenuItemUI.MAX_ACC_WIDTH, new Integer(acceleratorRect.width));
			}

			//Compare the accelarator widths.
			if(icon != null && icon.getIconWidth() > maxIconValue) {
				maxIconValue = icon.getIconWidth();
				p.putClientProperty(TinyMenuItemUI.MAX_ICON_WIDTH, new Integer(maxIconValue));
			}
		...
		Color background, Color foreground, int defaultTextIconGap)
	{
		...
		Integer maxValueInt = (Integer) p.getClientProperty(TinyMenuItemUI.MAX_ICON_WIDTH);
		...
		if(acceleratorText != null && !acceleratorText.equals("")) {
			...
			Container parent = menuItem.getParent();
			if(parent != null && parent instanceof JComponent) {
				Integer amaxValueInt = (Integer) p.getClientProperty(TinyMenuItemUI.MAX_ACC_WIDTH);
	...
	private void drawTinyTopMenuBorder(
		Graphics g, int x, int y, int w, int h, boolean selected) {
		
	}
	
	private void drawWinTopMenuBorder(
		Graphics g, int x, int y, int w, int h, boolean selected)
	{
		if(!selected) {
		...
		Graphics g, int x, int y, int w, int h, boolean selected)
	{
		...
		if(selected) {
			...
			// shadow
			drawXpShadow(g,
				ColorRoutines.darken(Theme.menuBorderColor[Theme.style].getColor(), 15),
				Theme.menuBarColor[Theme.style].getColor(),
				x + w - 7, y + 6, 6, h - 6);
		}
		else {
