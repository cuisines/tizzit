 ...
 * @version $Id$
 */
public final class PanJdbcRealm extends JPanel implements ConfigurationInterface {
	private static Logger log = Logger.getLogger(PanJdbcRealm.class);
	private static final long serialVersionUID = 7002935179625344641L;
	...
	public Integer getSelectedRealm() throws Exception {
		RealmJdbcValue val = (RealmJdbcValue) ((DropDownHolder) jComboBoxRealms.getSelectedItem()).getObject();
		return val.getJdbcRealmId();
	}
	...
	public void setExistingRealm(Integer realmId) {
		for (int i = 0; i < jComboBoxRealms.getItemCount(); i++) {
			...
			RealmJdbcValue val = (RealmJdbcValue) current.getObject();
			if (val.getJdbcRealmId().intValue() == realmId.intValue()) {
	...
	public PanJdbcRealm() {
		super();
		initialize();
		this.setBorder(new TitledBorder(rb.getString("panel.panelSafeguard.realm.sql")));
	}
