 ...
 * @version $Id$
 */
public class PanUnitGroupPerUser extends JPanel implements ExitListener, ActionListener, ReloadablePanel {
	...
	private UserValue getSelectedUser(int row) {
		if (row >= 0) { return (UserValue) tblUser.getModel().getValueAt(row, 5); }
		return null;
	}
	...
	private boolean contains(GroupValue[] gv, DropDownHolder groupDao) {
		for (int i = 0; i < gv.length; i++) {
			if (gv[i].getGroupId().equals(((GroupValue) groupDao.getObject()).getGroupId())) { return true; }
		}
		return false;
	}
	...
	public void save() {
		...
		if (currentSelected >= 0) saveChanges(currentSelected);
		...
		if (this.unitPickData.isModified() || this.groupPickData.isModified()) {
			try {
				...
				for (int i = 0; i < this.groupPickData.getLstLeftModel().getSize(); i++) {
					GroupValue group = (GroupValue) ((DropDownHolder) this.groupPickData.getLstLeftModel().getElementAt(i)).getObject();
				...
				if (!comm.isUserInRole(user, UserRights.SITE_ROOT)) {
					...
					for (int i = 0; i < this.unitPickData.getLstLeftModel().getSize(); i++) {
						UnitValue unit = (UnitValue) ((DropDownHolder) this.unitPickData.getLstLeftModel().getElementAt(i)).getObject();
	...
	private class UserListSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (tblUser.getSelectedRow() >= 0) {
				if (unitPickData.isModified() || groupPickData.isModified()) {
					int i = JOptionPane.showConfirmDialog(caller, rb.getString("dialog.wantToSave"),
				...
				try {
					if (comm.isUserInRole(user, UserRights.SITE_ROOT)) {
						...
						UnitValue[] vec = comm.getUnits4User(user.getUserName());
						unitPickData.getLstRightModel().removeAllElements();
						unitPickData.getLstLeftModel().removeAllElements();
						for (int i = 0; i < vec.length; i++) {
							DropDownHolder ddh = new DropDownHolder(vec[i], vec[i].getName());
							unitPickData.getLstLeftModel().addElement(ddh);
						}
						for (int i = 0; i < allUnits.length; i++) {
							if (!unitPickData.getLstLeftModel().contains(allUnits[i])) {
								unitPickData.getLstRightModel().addElement(allUnits[i]);
							}
						}
					}
					GroupValue[] grpHave = comm.getGroups4User(user.getUserName());
					groupPickData.getLstRightModel().clear();
					groupPickData.getLstLeftModel().clear();
					groupPickData.getLstRightModel().addAll(getDiffGroups(grpHave));
					if (grpHave != null) {
						DropDownHolder[] groups = new DropDownHolder[grpHave.length];
						for (int i = grpHave.length -1; i >= 0; i--) {
							groups[i] = new DropDownHolder(grpHave[i], grpHave[i].getGroupName());
						}
		...
		try {
			if (unitPickData.isModified() || groupPickData.isModified()) {
				int i = JOptionPane.showConfirmDialog(this, rb.getString("dialog.wantToSave"),
						rb.getString("dialog.title"), JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
				if (i == JOptionPane.YES_OPTION) {
					saveChanges(currentSelected);
