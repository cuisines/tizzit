...
<xsl:stylesheet version="1.0"
    ...
    <xsl:variable name="urlModulePreviewPresenter" select="'/flash/ModuleViewer.swf'"/>
    ...
    <xsl:variable name="flashConfig" select="document(concat('cocoon://',$language,'/',//flashobject/navigation/viewcomponent/url,'/content.xml'))/source/all/flashconfig"/>
    <xsl:variable name="moduleIdentifier" select="document(concat('cocoon://',$language,'/',//flashobject/navigation/viewcomponent/url,'/content.xml'))/source/all/content/identifier"/>
    ...
    <xsl:template match="/">
        <modules>
           ...
           <xsl:comment>
               ...
               <xsl:value-of select="concat('cocoon://',$language,'/',//flashobject/navigation/viewcomponent/url,'/content.xml')"/>               
                Loaded moduleconfig:
                <xsl:value-of select="concat('cocoon://',$language,'/',//flashobject/navigation/viewcomponent/url,'/content.xml')"/>                
               flashvars:      
