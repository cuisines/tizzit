...
<xsl:stylesheet version="1.0"
  ...
  <!--
    Columns group items layout made for aclasta osteo
  -->
  <xsl:template match="fi:group[fi:styling/@layout='advanced_2']" mode="group-layout">
        <xsl:apply-templates select="fi:items/*" mode="group-columns-content-advanced"/>
  </xsl:template>
  ...
  <!--
    group-columns-advanced made for aclasta-osteo
  -->
  <xsl:template match="fi:*" mode="group-columns-content-advanced">
     <div class="radio-checkboxgroup-item">
      <div class="radio-checkboxgroup-label"><xsl:apply-templates select="." mode="label"/></div>
      <div class="radio-checkboxgroup-element"><xsl:apply-templates select="."/></div>
    </div>
  </xsl:template>
