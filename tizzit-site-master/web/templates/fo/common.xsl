...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
    ...
    <!-- sollte von jedem überschrieben werden -->
	<xsl:template match="/">
    ...
    <xsl:template match="image">
    	<xsl:choose>
    		<xsl:when test=". = ''">
                <fo:external-graphic text-align="{@type}" width="{(@width div $dpifactor)}px" >
                    <xsl:attribute name="src">url('<xsl:value-of select="$ejbImgUrl"/>ejbimage?id=<xsl:value-of select="@src"/>')</xsl:attribute>
                </fo:external-graphic>
    		</xsl:when>
    		<xsl:otherwise>
    			<xsl:choose>
    				<xsl:when test="@type = 'center'">
    					<fo:block text-align="{@type}" space-after="3mm">
    							...
    							<fo:table-body>
    								<fo:table-row>
    									<fo:table-cell>
                        					...
                        					<fo:block text-align="{@type}" font-size="9pt" line-height="11pt" language="de" hyphenate="false">
                        						<xsl:choose>
    				...
    				<xsl:when test="@type = 'left'">
    					<fo:block text-align="{@type}" space-after="3mm">
    						<fo:table table-layout="fixed" width="100%">
    							<fo:table-column column-width="{(@width div $dpifactor)}px"/>
    							...
    							<fo:table-body>
    								<fo:table-row>
    									...
    									<fo:table-cell display-align="after">
    										<fo:block language="de" hyphenate="false" font-size="9pt" line-height="11pt"> 
    											<xsl:choose>
    												<xsl:when test="not(./child::*)">
    													<xsl:value-of select="." />
    												</xsl:when>
	...
	<xsl:template match="ul">
		<fo:block space-before.optimum="19pt" text-align="left"/>
	...
	<xsl:template match="li" mode="ul">
		<fo:list-item>
			...
			<fo:list-item-body start-indent="13pt" space-before.optimum="19pt" text-align="left">
				<fo:block>
					<xsl:apply-templates/>
				</fo:block>
			</fo:list-item-body>
	...
	<xsl:template match="li" mode="ol">
		<fo:list-item>
			...
			<fo:list-item-body start-indent="19pt" space-before.optimum="19pt" text-align="left">
				<fo:block>
					<xsl:apply-templates/>
				</fo:block>
			</fo:list-item-body>
