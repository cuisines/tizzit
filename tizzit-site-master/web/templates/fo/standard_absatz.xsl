...
<!--	<xsl:template match="item">
		<fo:block padding-top="5mm">
			<fo:table>
				...
				<fo:table-body>
					<fo:table-row>
						<fo:table-cell border-style="none" border-width="0mm" padding="1mm">
							<fo:block text-align="left">
								<xsl:if test="picture/image!=''">
									<xsl:apply-templates select="picture/image" mode="itempicture" />
								</xsl:if>
							</fo:block>
						</fo:table-cell>
						<fo:table-cell border-style="solid" border-width="0.2mm" padding="1mm">
							<xsl:apply-templates select="subhead" />
						</fo:table-cell>
					</fo:table-row>
				</fo:table-body>
...
-->
	...
	<xsl:template match="aggregation">
		<xsl:apply-templates select="include/include/content" />
	</xsl:template>

	<!--Titel-->
	<xsl:template match="title">
		<fo:block font-size="18pt" space-before.optimum="0pt" text-align="left" font-weight="bold" padding-after="5mm">
			<xsl:apply-templates/>
		</fo:block>
	</xsl:template>
	<!--	absatzüberschrift -->
	<xsl:template match="subhead">
		<fo:block font-size="14pt" font-weight="bold" padding-after="3mm">
			<xsl:value-of select="." />
		</fo:block>
	</xsl:template>
	...
	<!--	fette Schrift -->
	<xsl:template match="b | subline  | subtextline | textline">
		<fo:inline font-size="12pt" font-weight="bold">
			<xsl:apply-templates/>
		</fo:inline>
	</xsl:template>
	<!-- Absatzbilder -->
	<xsl:template match="image" mode="itempicture">
		<fo:block text-align="left">
			<fo:external-graphic>
				<xsl:attribute name="src">http://localhost:/cms/img/ejbimage?id=<xsl:value-of select="@src"/>&amp;typ=t</xsl:attribute>
	...
	<xsl:template match="include[@type='person'][@listtype='Normal' or @listtype='normal'] | include[@type='person'][@listtype='Listenform' or @listtype='list'] ">
...
<xsl:template match="unitMembersList">
	...
	<xsl:if test=".!='0'">
		<fo:block>
			<fo:external-graphic>
				<xsl:attribute name="src">http://localhost:/ukw/img/ejbimage?id=<xsl:value-of select="."/>&amp;typ=s</xsl:attribute>
