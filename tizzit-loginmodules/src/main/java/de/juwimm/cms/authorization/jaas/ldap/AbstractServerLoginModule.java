...
public abstract class AbstractServerLoginModule implements LoginModule {
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler, Map sharedState, Map options) {
		...
		if (log.isTraceEnabled()) log.trace("initialize, instance=@" + System.identityHashCode(this));

		//log securityDomain, if set. 
		log.trace("Security domain: " + (String) options.get(SecurityConstants.SECURITY_DOMAIN_OPTION));
		...
		if (passwordStacking != null && passwordStacking.equalsIgnoreCase("useFirstPass")) useFirstPass = true;
	...
	protected Group createGroup(String name, Set principals) {
		...
		while (iter.hasNext()) {
			...
			if ((next instanceof Group) == false) continue;
