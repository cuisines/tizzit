...
public class HTMLParser implements HTMLParserConstants {
	...
	public String getTitle() throws IOException, InterruptedException {
		...
		while (true) {
			synchronized (this) {
				if (titleComplete || pipeInStream.full()) break;
				wait(10);
			}
		}
	...
	public String getSummary() throws IOException, InterruptedException {
		...
		while (true) {
			synchronized (this) {
				if (summary.length() >= SUMMARY_LENGTH || pipeInStream.full()) break;
				wait(10);
			}
		}
		...
		String tit = getTitle();
		if (sum.startsWith(tit) || sum.equals("")) return tit;
		return sum;
	}
			...
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case ArgEquals:
					...
					switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
						...
						case ArgQuote2:
							t2 = ArgValue();
							if (inImg && t1.image.equalsIgnoreCase("alt") && t2 != null) addText("[" + t2.image + "]");
